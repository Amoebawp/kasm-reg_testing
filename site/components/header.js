...
export default function Header({ searchText, changeSearch }) {
  ...
  return (
    <header className="relative font-light overflow-hidden bg-gradient-to-tr from-[#a3be8c] to-[#8fbcbb] p-8 xl:px-32 text-[#e5e9f0] gap-5 md:gap-0 flex flex-wrap justify-center items-center">
      <Bubbles />
      ...
      <nav className='relative z-10 mx-12'>
        <a href="/" className={'p-4 inline-block rounded-full border border-solid' + (router.pathname.startsWith("/") ? ' border-white/30' : ' border-transparent')}>Library</a>
      ...
      <button className='p-4 relative z-10 px-5 bg-[#3b4252]/70 border-t border-white/20 border-solid hover:bg-slate-900 transition shadow-lg m-2 rounded items-center text-white/70 flex cursor-pointer' onClick={() => { copyToClipboard() }}>
